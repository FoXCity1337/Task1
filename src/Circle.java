...
public class Circle {
    ...
    public void crossing(int x1, int y1, int r1, int x2, int y2, int r2) { //метод пересечения окружностей

        double AC = Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));//Нахожу расстояние между центрами двух окружностей
        boolean cross = r1 + r2 > AC && r1 + AC > r2 && r2 + AC > r1; /*Окружности пересекаются в случае если являются длинами сторон некоторого треугольника,
